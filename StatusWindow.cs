...
namespace ScienceChecklist
{
	class StatusWindow : Window<ScienceChecklistAddon>
	{
		...
		public void RefreshFilter(object sender, EventArgs e)
		{
			...
			Vessel v = HighLogic.LoadedScene == GameScenes.FLIGHT ? FlightGlobals.ActiveVessel : null;
			if (v)
			{
				_moduleScienceExperiments = v.FindPartModulesImplementing<ModuleScienceExperiment>();
				if (DMagic.Installed)
				{
					_DMModuleScienceAnimates = _moduleScienceExperiments.Where(
						x => DMagic.IsScienceAnimate(x)).ToList();
					_DMModuleScienceAnimateGenerics = _moduleScienceExperiments.Where(
						x => DMagic.IsScienceAnimateGeneric(x)).ToList();
				}
			}
		...
		public void UpdateSituation(object sender, NewSituationData e)
		{
			...
			foreach (var experiment in _filter.DisplayScienceInstances)
			{
				...
				// Always pop UI for Kerbal experiments
				var id = experiment.ScienceExperiment.id;
				if (id == "crewReport" || id == "surfaceSample" || id == "evaReport")
				{
			...
			if (_parent.Config.PlayNoise)
				NoiseEvent?.Invoke(this, EventArgs.Empty);
			if (_parent.Config.StopTimeWarp || _parent.Config.PlayNoise)
				ScreenMessages.PostScreenMessage("New Situation: "
					+ _filter.CurrentSituation.Description);
		}
		#endregion
		...
		public bool CanRunExperiment(ScienceInstance s, bool runSingleUse = true)
		{
			...
			if (dlm != null && dlm.Any())
			{
				var NewDMagicInstance = DMagic.ScienceAnimateGeneric;
		...
		public void RunExperiment(ScienceInstance s, bool runSingleUse = true)
		{
			...
			if (lm != null && lm.Any())
			{
				var api = DMagic.ScienceAnimateGeneric;
				m = lm.FirstOrDefault(x => api.canConduct(x, runSingleUse));
				if (m != null)
				{
					_logger.Debug("Running DMModuleScienceAnimateGenerics Experiment " + m.experimentID + " on part " + m.part.partInfo.name);
					api.gatherScienceData(m, !_parent.Config.ShowResultsWindow);
					return;
				}
				_logger.Warning("No suitable DMModuleScienceAnimateGenerics Experiment");
			}

			// If possible run with DMagic DMAPI
			if (_DMModuleScienceAnimates?.Count > 0)
			{
				var api = DMagic.ScienceAnimate;
				if (api != null)
				{
					IEnumerable<ModuleScienceExperiment> lm2 = _DMModuleScienceAnimates.Where(x => x.experimentID == s.ScienceExperiment.id);
					if (lm2.Any())
					{
						m = lm2.FirstOrDefault(x =>	!x.Inoperable && api.canConduct(x, runSingleUse));
						if (m != null)
						{
						...
						_logger.Warning("No suitable DMModuleScienceAnimates Experiment");
					}
				}
			}
		...
		{
			if (_DMModuleScienceAnimateGenerics?.Count > 0)
			{
				var NewDMagicInstance = DMagic.ScienceAnimateGeneric;
